...
(function ($, undefined) {
  acf.FieldObject = acf.Model.extend({
    ...
    onChanged: function (e, $el, name, value) {
      ...
      // render
      if (
        ["menu_order", "label", "required", "name", "type", "key"].indexOf(
          name
        ) > -1
      ) {
        this.render();
    ...
    move: function () {
      ...
      if (changed) {
        alert(
          acf.__("This field cannot be moved until its changes have been saved")
        );
        return;
      }
    ...
    addFieldActions: function (action) {
      ...
      var singleCallback = function (fieldObject /*, arg1, arg2, etc*/) {
        ...
        variations.map(function (variation) {
          args[0] =
            singleAction + "/" + variation + "=" + fieldObject.get(variation);
          acf.doAction.apply(null, args);
        });
